...
- `get_customer` — Retrieve one MoonClerk customer by its numeric ID.
- `get_form` — Retrieve one MoonClerk payment form by its numeric ID.
- `get_payment` — Retrieve one MoonClerk payment by its numeric ID.
- `list_customers` — List MoonClerk customers with official filters for form, checkout date, next payment date, and status.
- `list_forms` — List MoonClerk payment forms with official pagination parameters.
- `list_payments` — List MoonClerk payments with official filters for form, customer, payment date, and status.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change MoonClerk state — confirm the exact payload and effect with the user before running.**
