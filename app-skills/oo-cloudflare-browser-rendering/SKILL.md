...
- **`scope_missing` / `credential_expired` / `app_not_ready` / `app_not_found`** — Cloudflare Browser Run is not connected, or the connection expired or lacks a scope. Connect once (auth type: API key, OAuth2) at:
