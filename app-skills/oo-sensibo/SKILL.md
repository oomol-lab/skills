...
- [`get_ac_states`](actions/get_ac_states.md) — Get current and previous AC states for one Sensibo device.
- [`get_device`](actions/get_device.md) — Get detailed information for one Sensibo device.
- [`list_devices`](actions/list_devices.md) — List Sensibo devices linked to the authenticated user.
- [`set_ac_state`](actions/set_ac_state.md) — Set the full AC state for one Sensibo device.

## Safety

- Read actions (get / list / search) are safe to run directly.
- **Create, update, send, or post actions change Sensibo state — confirm the exact payload and effect with the user before running.**
