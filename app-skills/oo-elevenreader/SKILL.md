...
- `get_models` — List available ElevenLabs speech synthesis models for reading text aloud.
- `get_user_info` — Get the current ElevenReader user profile and subscription snapshot.
- `get_voice` — Get one ElevenLabs voice by voice ID before using it to read text aloud.
- `read_text` — Convert text to speech with an ElevenLabs voice and upload the generated audio to connector transit storage.
- `search_voices` — Search available ElevenLabs voices with pagination and filters useful for reading text aloud.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change ElevenReader state — confirm the exact payload and effect with the user before running.**
