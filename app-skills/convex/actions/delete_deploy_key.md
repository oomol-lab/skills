# Convex · `delete_deploy_key`

Delete a deploy key for a Convex deployment.

- **Service**: `convex`
- **Action**: `delete_deploy_key`
- **Action id**: `convex.delete_deploy_key`
- **Required scopes**: convex.deploy_keys.write

## Inspect the schema

Always fetch the authoritative input/output schema before building a payload — fields and defaults can change upstream:

```bash
oo connector schema "convex" --action "delete_deploy_key"
```

## Run

```bash
oo connector run "convex" --action "delete_deploy_key" --data '{}' --json
```

Replace `{}` with a JSON object that matches the input schema. The response is `{ "data": ..., "meta": { "executionId": "..." } }`.

> **Destructive action.** This removes or overwrites Convex data. Always confirm the target and get explicit user approval before running.
