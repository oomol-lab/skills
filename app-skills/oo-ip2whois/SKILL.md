...
- `lookup_domain` — Look up WHOIS registration details for a domain.
- `lookup_hosted_domains` — List hosted domains associated with an IP address.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change IP2WHOIS state — confirm the exact payload and effect with the user before running.**
