# ClickUp · `move_task_to_home_list`

Move a ClickUp task to a new home list.

- **Service**: `clickup`
- **Action**: `move_task_to_home_list`
- **Action id**: `clickup.move_task_to_home_list`
- **Required scopes**: clickup.write

## Inspect the schema

Always fetch the authoritative input/output schema before building a payload — fields and defaults can change upstream:

```bash
oo connector schema "clickup" --action "move_task_to_home_list"
```

## Run

```bash
oo connector run "clickup" --action "move_task_to_home_list" --data '{}' --json
```

Replace `{}` with a JSON object that matches the input schema. The response is `{ "data": ..., "meta": { "executionId": "..." } }`.

> **Write action.** This changes ClickUp state. Confirm the exact payload and intended effect with the user before running.
