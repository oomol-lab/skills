...
- `get_astronomy` — Get sunrise, sunset, moonrise, moonset, and moon phase data for a location.
- `get_timezone` — Get time zone information by IP address, coordinates, location, or time zone name.
- `lookup_ip` — Look up IP geolocation data with optional field controls.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change IPGeolocation.io state — confirm the exact payload and effect with the user before running.**
