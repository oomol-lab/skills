...
- `create_form` — Create a new Formcarry form with basic notification, redirect, and storage settings. [write]
- `delete_form` — Delete an existing Formcarry form by ID. [destructive]
- `list_submissions` — List submissions for a Formcarry form with the documented pagination, sorting, and filtering query parameters.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Formcarry state — confirm the exact payload and effect with the user before running.**
