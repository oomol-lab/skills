...
- `create_contact` — Create a new sevdesk contact using the official contact payload fields. [write]
- `delete_contact` — Delete a sevdesk contact by its identifier. [destructive]
- `get_contact` — Get one sevdesk contact by its identifier.
- `list_contacts` — List sevdesk contacts with optional customer number, pagination, and embed options.
- `update_contact` — Update an existing sevdesk contact using the official contact update payload fields. [write]

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change sevdesk state — confirm the exact payload and effect with the user before running.**
