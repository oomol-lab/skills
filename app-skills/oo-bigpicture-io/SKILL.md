...
- `find_company_by_domain` — Look up a BigPicture company profile by domain name.
- `find_company_by_ip` — Look up the company associated with an IPv4 or IPv6 address using BigPicture.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change BigPicture.io state — confirm the exact payload and effect with the user before running.**
