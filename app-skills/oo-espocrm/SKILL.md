...
- `create_record` — Create one EspoCRM record for the specified entity type. [write]
- `delete_record` — Delete one EspoCRM record by entity type and record identifier. [destructive]
- `get_app_user` — Get the current EspoCRM user data for the configured connection, including ACL and preferences when returned.
- `get_metadata` — Get EspoCRM application metadata, optionally narrowed to one metadata path.
- `get_record` — Get one EspoCRM record by entity type and record identifier.
- `list_records` — List EspoCRM records for an entity type with optional pagination, sorting, and where clauses.
- `update_record` — Update selected fields on one EspoCRM record. [write]

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change EspoCRM state — confirm the exact payload and effect with the user before running.**
