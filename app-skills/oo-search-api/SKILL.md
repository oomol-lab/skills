...
- `get_account_info` — Retrieve account usage statistics for the connected SearchApi API key.
- `get_cached_search_html` — Retrieve one cached SearchApi search result in HTML format by search identifier.
- `get_cached_search_json` — Retrieve one cached SearchApi search result in JSON format by search identifier.
- `get_locations` — Look up canonical SearchApi locations for geo-targeted search queries.
- `search` — Run one SearchApi search request with the first-pass common query parameters.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change SearchApi state — confirm the exact payload and effect with the user before running.**
