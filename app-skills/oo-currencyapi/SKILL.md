...
- `convert_currency` — Convert a monetary amount into one or more currencies with currencyapi.
- `get_api_status` — Retrieve current currencyapi account quota usage.
- `get_historical_rates` — Retrieve historical exchange rates for a specific date from currencyapi.
- `get_latest_rates` — Retrieve the latest exchange rates from currencyapi.
- `get_supported_currencies` — Retrieve supported currency metadata from currencyapi.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change currencyapi state — confirm the exact payload and effect with the user before running.**
