...
- `get_item` — Get a single BoxHero inventory item by item identifier.
- `get_team_info` — Get the linked BoxHero team information and team mode.
- `list_items` — List BoxHero inventory items with optional location filters and cursor pagination.
- `list_locations` — List BoxHero locations available to the linked team.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change BoxHero state — confirm the exact payload and effect with the user before running.**
