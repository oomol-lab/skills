...
- `get_client` — Get one BidSketch client by client ID.
- `get_proposal` — Get one BidSketch proposal by proposal ID.
- `get_proposal_content` — Get one BidSketch proposal with its grouped sections and fees.
- `list_client_proposals` — List proposals that belong to one BidSketch client.
- `list_clients` — List clients available in the authenticated BidSketch account.
- `list_proposals` — List proposals available in the authenticated BidSketch account.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change BidSketch state — confirm the exact payload and effect with the user before running.**
