...
- `enrich_organization` — Enrich an Apollo organization by domain.
- `enrich_person` — Enrich an Apollo person with the first-pass matching inputs.
- `get_api_usage_stats` — Retrieve Apollo API usage statistics for the current team and key.
- `search_organizations` — Search Apollo organizations with the first-pass organization filters.
- `search_people` — Search Apollo people with the first-pass prospecting filters.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Apollo state — confirm the exact payload and effect with the user before running.**
