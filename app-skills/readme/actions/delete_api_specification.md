# ReadMe · `delete_api_specification`

Delete one ReadMe API specification by ID.

- **Service**: `readme`
- **Action**: `delete_api_specification`
- **Action id**: `readme.delete_api_specification`

## Inspect the schema

Always fetch the authoritative input/output schema before building a payload — fields and defaults can change upstream:

```bash
oo connector schema "readme" --action "delete_api_specification"
```

## Run

```bash
oo connector run "readme" --action "delete_api_specification" --data '{}' --json
```

Replace `{}` with a JSON object that matches the input schema. The response is `{ "data": ..., "meta": { "executionId": "..." } }`.

> **Destructive action.** This removes or overwrites ReadMe data. Always confirm the target and get explicit user approval before running.
