...
- `create_audio` — Generate base64 audio from one or more voice/text segments. [write]
- `list_voices` — List available AiVOOV voices, optionally filtered by language code.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change AiVOOV state — confirm the exact payload and effect with the user before running.**
