...
- `create_submission` — Create a submission on a Jotform form using question IDs and answer values. [write]
- `get_current_user` — Get the current Jotform account associated with the authenticated API key.
- `get_form` — Get one Jotform form by form ID.
- `get_submission` — Get one Jotform submission by submission ID.
- `list_form_questions` — List the question definitions configured on one Jotform form.
- `list_form_submissions` — List submissions for one Jotform form.
- `list_forms` — List forms available to the authenticated Jotform account.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Jotform state — confirm the exact payload and effect with the user before running.**
