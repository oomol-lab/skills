...
- `get_web_form_definition` — Fetch the visible HelloLeads web form definition for the connected Web Form Key and report whether reCAPTCHA v2 is enabled.
- `submit_web_form` — Submit one HelloLeads web form lead with JSON field values, excluding reCAPTCHA and file-upload workflows. [write]

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change HelloLeads state — confirm the exact payload and effect with the user before running.**
