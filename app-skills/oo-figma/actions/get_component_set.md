# Figma · `get_component_set`

Get metadata for a published Figma component set by key.

- **Service**: `figma`
- **Action**: `get_component_set`
- **Action id**: `figma.get_component_set`
- **Required scopes**: library_assets:read

## Inspect the schema

Always fetch the authoritative input/output schema before building a payload — fields and defaults can change upstream:

```bash
oo connector schema "figma" --action "get_component_set"
```

## Run

```bash
oo connector run "figma" --action "get_component_set" --data '{}' --json
```

Replace `{}` with a JSON object that matches the input schema. The response is `{ "data": ..., "meta": { "executionId": "..." } }`.

> **Write action.** This changes Figma state. Confirm the exact payload and intended effect with the user before running.
