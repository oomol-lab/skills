...
- `google_maps_search` — Run a Google Maps search through SerpApi.
- `google_news_search` — Run a Google News search through SerpApi.
- `google_search` — Run a Google web search through SerpApi.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change SerpApi state — confirm the exact payload and effect with the user before running.**
