...
- [`find_address`](actions/find_address.md) — Autocomplete addresses from a partial query and return the official AddressZen suggestions.
- [`get_key_availability`](actions/get_key_availability.md) — Retrieve the current AddressZen key availability and context list.
- [`retrieve_address_usa`](actions/retrieve_address_usa.md) — Retrieve a USA-formatted address from an AddressZen suggestion identifier and return the official response wrapper.

## Safety

- Read actions (get / list / search) are safe to run directly.
- **Create, update, send, or post actions change AddressZen state — confirm the exact payload and effect with the user before running.**
