# Gamma · `create_generation_from_template`

Create an asynchronous Gamma generation from an existing Gamma template.

- **Service**: `gamma`
- **Action**: `create_generation_from_template`
- **Action id**: `gamma.create_generation_from_template`

## Inspect the schema

Always fetch the authoritative input/output schema before building a payload — fields and defaults can change upstream:

```bash
oo connector schema "gamma" --action "create_generation_from_template"
```

## Run

```bash
oo connector run "gamma" --action "create_generation_from_template" --data '{}' --json
```

Replace `{}` with a JSON object that matches the input schema. The response is `{ "data": ..., "meta": { "executionId": "..." } }`.

> **Write action.** This changes Gamma state. Confirm the exact payload and intended effect with the user before running.
