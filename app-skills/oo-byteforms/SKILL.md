...
- `get_form` — Get one ByteForms form by form ID.
- `list_form_responses` — List responses for one ByteForms form with the documented cursor, query, order, and limit parameters.
- `list_forms` — List forms available to the authenticated ByteForms account.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change ByteForms state — confirm the exact payload and effect with the user before running.**
