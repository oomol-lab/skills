...
- `create_list_contact` — Create a contact in an EmailOctopus mailing list. [write]
- `delete_list_contact` — Delete a contact from an EmailOctopus mailing list. [destructive]
- `get_campaign` — Fetch a single EmailOctopus campaign by ID.
- `get_list` — Fetch a single EmailOctopus mailing list by ID.
- `get_list_contact` — Fetch a single contact from an EmailOctopus mailing list.
- `list_campaigns` — List EmailOctopus campaigns available to the current API key.
- `list_list_contacts` — List contacts in a specific EmailOctopus mailing list.
- `list_lists` — List EmailOctopus mailing lists available to the current API key.
- `update_list_contact` — Update a contact in an EmailOctopus mailing list. [write]

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change EmailOctopus state — confirm the exact payload and effect with the user before running.**
