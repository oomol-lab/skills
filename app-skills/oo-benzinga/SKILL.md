...
- `get_consensus_ratings` — Get Benzinga consensus analyst ratings and price target data for a ticker.
- `list_analyst_ratings` — List Benzinga analyst rating calendar events with optional date and symbol filters.
- `list_earnings` — List Benzinga earnings calendar events with optional date and symbol filters.
- `list_news_channels` — List Benzinga news channels that can be used to filter news feeds.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Benzinga state — confirm the exact payload and effect with the user before running.**
