# Baidu Qianfan · `create_completion`

Create a non-streaming fill-in-the-middle completion with Baidu Qianfan.

- **Service**: `qianfan`
- **Action**: `create_completion`
- **Action id**: `qianfan.create_completion`

## Inspect the schema

Always fetch the authoritative input/output schema before building a payload — fields and defaults can change upstream:

```bash
oo connector schema "qianfan" --action "create_completion"
```

## Run

```bash
oo connector run "qianfan" --action "create_completion" --data '{}' --json
```

Replace `{}` with a JSON object that matches the input schema. The response is `{ "data": ..., "meta": { "executionId": "..." } }`.

> **Write action.** This changes Baidu Qianfan state. Confirm the exact payload and intended effect with the user before running.
