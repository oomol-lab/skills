...
- `get_credits` — Retrieve the current Veriphone verification credit summary for the account.
- `verify_phone_number` — Verify whether a phone number is valid and return its carrier and region data.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Veriphone state — confirm the exact payload and effect with the user before running.**
