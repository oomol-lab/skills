...
Operate **Trigger.dev** through your OOMOL-connected account. This skill calls the `trigger_dev` connector with the [oo CLI](https://github.com/oomol-lab/oo-cli); OOMOL injects credentials server-side, so you never handle raw tokens.

## Running an action

Assume the user has already installed the oo CLI, signed in, and connected Trigger.dev. **Do not run `oo auth login` or open the connection URL proactively — just run the action.** Fall back to [First-time setup](#first-time-setup) only when a command actually fails with an auth or connection error.
...
- **Actions tagged `[write]` change Trigger.dev state — confirm the exact payload and effect with the user before running.**
