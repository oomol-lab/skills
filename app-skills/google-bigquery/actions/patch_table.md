# Google BigQuery · `patch_table`

Patch BigQuery table metadata.

- **Service**: `google_bigquery`
- **Action**: `patch_table`
- **Action id**: `google_bigquery.patch_table`
...
```bash
oo connector schema "google_bigquery" --action "patch_table"
```

## Run

```bash
oo connector run "google_bigquery" --action "patch_table" --data '{}' --json
```
