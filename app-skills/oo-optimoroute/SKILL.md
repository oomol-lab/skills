...
- `create_or_update_orders` — Create, update, merge, or sync one or more OptimoRoute orders in one request. [write]
- `delete_orders` — Delete one or more OptimoRoute orders by orderNo or id. [destructive]
- `get_orders` — Retrieve one or more OptimoRoute orders by orderNo or id.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change OptimoRoute state — confirm the exact payload and effect with the user before running.**
