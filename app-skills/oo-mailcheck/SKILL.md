...
- `get_status` — Retrieve UserCheck account information, plan details, and current API usage.
- `validate_domain` — Validate a domain and return UserCheck domain-level risk and MX signals.
- `verify_email` — Validate a single email address and return UserCheck deliverability and risk signals.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Mailcheck state — confirm the exact payload and effect with the user before running.**
