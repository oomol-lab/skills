# Google Calendar · `update_acl_rule`

Replace writable fields on a Google Calendar ACL rule.

- **Service**: `googlecalendar`
- **Action**: `update_acl_rule`
- **Action id**: `googlecalendar.update_acl_rule`
- **Required scopes**: googlecalendar.acl.write

## Inspect the schema

Always fetch the authoritative input/output schema before building a payload — fields and defaults can change upstream:

```bash
oo connector schema "googlecalendar" --action "update_acl_rule"
```

## Run

```bash
oo connector run "googlecalendar" --action "update_acl_rule" --data '{}' --json
```

Replace `{}` with a JSON object that matches the input schema. The response is `{ "data": ..., "meta": { "executionId": "..." } }`.

> **Write action.** This changes Google Calendar state. Confirm the exact payload and intended effect with the user before running.
