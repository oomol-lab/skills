...
- `get_account` — Get Freshdesk account details for the current API key.
- `get_ticket` — Get a single Freshdesk ticket by identifier.
- `list_ticket_conversations` — List conversations attached to a Freshdesk ticket.
- `list_tickets` — List Freshdesk tickets with optional filters and include expansions.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Freshdesk state — confirm the exact payload and effect with the user before running.**
