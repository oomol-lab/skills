...
- `get_check` — Retrieve a detailed Pingdom uptime check by ID.
- `get_credits` — Retrieve Pingdom account check and SMS credit information.
- `list_checks` — List Pingdom uptime checks with optional pagination and tag filters.
- `list_probes` — List Pingdom probe servers and their location details.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Pingdom state — confirm the exact payload and effect with the user before running.**
