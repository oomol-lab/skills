...
- `create_data` — Create a Dovetail data record inside a specific project. [write]
- `export_data` — Export a Dovetail data record in HTML or Markdown format.
- `get_data` — Get one Dovetail data record by identifier.
- `get_token_info` — Get metadata for the current Dovetail personal API key.
- `list_data` — List Dovetail data records with pagination and common filters.
- `list_projects` — List Dovetail projects with pagination and title filtering.
- `update_data` — Update the title or fields of a Dovetail data record. [write]

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Dovetail state — confirm the exact payload and effect with the user before running.**
