...
- `find_associated_domains` — Find domains associated with one hostname in SecurityTrails.
- `get_domain` — Get current DNS and domain details for one hostname from SecurityTrails.
- `get_domain_ssl` — Get SSL certificate details for one hostname from SecurityTrails.
- `get_subdomains` — List known subdomains for one hostname from SecurityTrails.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change SecurityTrails state — confirm the exact payload and effect with the user before running.**
