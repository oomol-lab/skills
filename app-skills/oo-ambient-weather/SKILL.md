...
- `get_device_history` — Get recent historical observation records for an Ambient Weather device.
- `get_latest_device_data` — Get the latest observation record for an Ambient Weather device.
- `list_devices` — List Ambient Weather devices linked to the connected account.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Ambient Weather state — confirm the exact payload and effect with the user before running.**
