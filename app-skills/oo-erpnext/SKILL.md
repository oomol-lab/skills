...
- `create_document` — Create one ERPNext document for the specified DocType. [write]
- `delete_document` — Delete one ERPNext document by DocType and name. [destructive]
...
- `set_document_value` — Set one field value on an ERPNext document and return the updated document. [write]
- `update_document` — Update selected fields on one ERPNext document. [write]

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change ERPNext state — confirm the exact payload and effect with the user before running.**
