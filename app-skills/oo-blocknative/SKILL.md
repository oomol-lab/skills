...
- `get_gas_price_distribution` — Get the current Blocknative gas-price distribution for Ethereum mainnet.
- `get_gas_prices` — Get Blocknative gas-price estimates for the default chain or a selected network.
- `list_gas_oracles` — List the gas-oracle metadata exposed by Blocknative across supported chains.
- `list_supported_chains` — List the chains currently supported by the Blocknative Gas Platform.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Blocknative state — confirm the exact payload and effect with the user before running.**
