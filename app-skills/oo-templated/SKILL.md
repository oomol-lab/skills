...
- `create_render` — Create a Templated render from one template with optional shared layer overrides and image or PDF output settings. [write]
- `delete_render` — Delete a Templated render by its render ID. [destructive]
- `get_account` — Get the current Templated account associated with the API key.
- `get_render` — Retrieve a single Templated render by its render ID.
- `get_template` — Retrieve a single Templated template by its template ID.
- `list_renders` — List all renders owned by the current Templated account.
- `list_templates` — List Templated templates with optional filters for name, dimensions, and tags.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Templated state — confirm the exact payload and effect with the user before running.**
