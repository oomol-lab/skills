...
- `get_booking` — Get one booking by its Headout booking id.
- `get_product` — Get one Headout product with variants and pricing.
- `list_bookings` — List bookings accessible to the current Headout API key.
- `list_categories_by_city` — List Headout categories for a given city.
- `list_cities` — List active Headout cities.
- `list_inventory_by_variant` — List Headout inventory rows for one variant.
- `list_products_by_category` — List Headout product listings for a category.
- `list_products_by_city` — List Headout product listings for a city.

## Safety

- Untagged actions are reads (get / list / search) — safe to run directly.
- **Actions tagged `[write]` change Headout state — confirm the exact payload and effect with the user before running.**
